...
<div id="maininner"><!-- maininner (s)-->
    ...
    <table class="tbl_lst tdleft">
        ...
        <form name="${searchVO.cmd eq 'write' ? 'writeForm' : 'updateForm'}"
              id="${searchVO.cmd eq 'write' ? 'writeForm' : 'updateForm'}" method="post" enctype="multipart/form-data">
            ...
            <c:if test="${ searchVO.cmd eq 'update' }">
                ...
                <tr>
                    ...
                    <td class="tl" colspan="3">
                        <select name="bbscateId" id="bbscate_id" title="구분 선택" style="margin-bottom:5px;width:150px">
            ...
            <tr>
                ...
                <tr>
                    ...
                    <td colspan="3">
                        ...
                        <input type="file" name="file_0" id="egovComFileUploader" title="파일"
                               onchange="loadname(this,'preThumbnail',0)">
                        <img style="width: 200px;height: 200px;"
                             src="${searchVO.cmd eq 'write' ? '/images/sub/blank2.gif' : thumbnail}" name="preThumbnail"
                             class="preThumbnail" id="preThumbnail" alt="">
                        <c:if test="${searchVO.cmd eq 'update' && result.thumbAtchFileId!='0' && result.thumbAtchFileId!=''  }">
                            <div id="thumb"><br>
                                ...
                                <a href="#" onclick="fileDel2('thumb','<c:out value="${thumbFile.atchFileId}"/>','<c:out
            ...
            <c:if test="${bbsManageVO.bbsTyCode ne 'BBSTYCODE2'}">
                <input type="file" name="file_0" id="egovComFileUploader" title="파일" style="display:none;">
            </c:if>
            <c:if test="${bbsManageVO.bbsTyCode eq 'BBSTYCODE4'}">
                <input type="hidden" name="secAt" value="Y"/>
            </c:if>
            <c:if test="${bbsManageVO.bbsTyCode ne 'BBSTYCODE4'}">
                <input type="hidden" name="secAt" value="N"/>
            </c:if>
            <!-- 게시판관리 파일개수 제한이 0보다 크고 파일사용여부가 Y인 경우 -->
            <c:if test="${bbsManageVO.atchPosblFileNumber > 0 && bbsManageVO.fileAtchPosblYn ne 'N'}">

                <c:set var="file1Status" value="false"/>
                <c:set var="file2Status" value="false"/>
                <c:set var="file3Status" value="false"/>
                <c:set var="file4Status" value="false"/>
                ...
                <c:choose>
                    <c:when test="${searchVO.cmd eq 'update' }">
                        <c:forEach var="file" items="${fileList}" varStatus="status">
                            <tr>
                                ...
                                <td class="tl" colspan="3">
                                    <div class="filewrp">
                                        <c:choose>
                                            <c:when test="${file.fileSn eq 1}">
                                                <c:set var="file1Status" value="true"/>
                                            </c:when>
                                            <c:when test="${file.fileSn eq 2}">
                                                <c:set var="file2Status" value="true"/>
                                            </c:when>
                                            <c:when test="${file.fileSn eq 3}">
                                                <c:set var="file3Status" value="true"/>
                                            </c:when>
                                            <c:when test="${file.fileSn eq 4}">
                                                <c:set var="file4Status" value="true"/>
                                            </c:when>
                                        ...
                                        <input type="text" readonly title="File Route" id="file_${file.fileSn}_txt"
                                               value="${file.orignlfileName}">
                                        <label class="mr5">
                                            파일등록
                                            <input type="file" name="file_${file.fileSn}"
                                                   onchange="javascript:document.getElementById('file_${file.fileSn}_txt').value=this.value">
                                        </label>
                                            <%--<label class="pls">--%>
                                            <%--파일추가--%>
                                            <%--</label>--%>
                                        <label class="del mr5">
                                            <a href="#" class="fileDel"
                                               onclick="fileDel2('attach','<c:out value="${file.atchFileId}"/>','<c:out
                                                       value="${file.fileSn}"/>'); return false;">파일삭제
                                            </a>
                                        </label>
                                    </div>
                                </td>
                        ...
                        <c:set var="AtchCnts" value="${fn:length(fileList)}"/>
                        <c:if test="${fn:length(fileList) < bbsManageVO.atchPosblFileNumber }">
                            <c:forEach begin="1" end="${bbsManageVO.atchPosblFileNumber }"
                                       step="1" varStatus="status" var="cnts">
                                <c:set var="Nums" value="0"/>
                                <c:if test="${cnts == 1 && !file1Status}">
                                    <c:set var="Nums" value="1"/>
                                </c:if>

                                <c:if test="${cnts == 2 && !file2Status}">
                                    <c:set var="Nums" value="2"/>
                                </c:if>

                                <c:if test="${cnts == 3 && !file3Status}">
                                    <c:set var="Nums" value="3"/>
                                </c:if>

                                <c:if test="${cnts == 4 && !file4Status}">
                                    <c:set var="Nums" value="4"/>
                                </c:if>
                                <c:if test="${Nums != 0}">
                                    <c:set var="AtchCnts" value="${AtchCnts+1}"/>
                                    <tr>
                                        <th scope="row">첨부파일 ${status.count+fn:length(fileList)}</th>
                                        <td class="tl" colspan="3">

                                            <div class="filewrp">
                                                <input type="text" readonly title="File Route" id="file_${Nums}_txt">
                                                <label class="mr5">
                                                    파일등록
                                                    <input type="file" name="file_${Nums}" class="file_${Nums}"
                                                           onchange="javascript:document.getElementById('file_${Nums}_txt').value=this.value">
                                                </label>
                                                <label class="del mr5">
                                                    <a href="#" class="fileDel"
                                                       onclick="fileDel('${Nums}'); return false;">파일삭제
                                                    </a>
                                                </label>
                                            </div>
                                                <%--<input type="file" name="file_${status.count+fn:length(fileList)}"--%>
                        ...
                        <c:forEach begin="1" end="${bbsManageVO.atchPosblFileNumber }" step="1" varStatus="status">
                            <tr>
                                ...
                                <td class="tl" colspan="3">
                                    ...
                                    <div class="filewrp">
                                        <input type="text" readonly title="File Route" id="file_${status.count}_txt">
                                        <label class="mr5">
                                            파일등록
                                            <input type="file" name="file_${status.count}" class="file_${status.count}"
                                                   onchange="javascript:document.getElementById('file_${status.count}_txt').value=this.value">
                                        </label>
                                        <label class="del mr5">
                                            <a href="#" class="attachCk fileDel"
                                               onclick="fileDel('${status.count}'); return false;">파일삭제
                                            </a>
                                        </label>
                                    </div>
            ...
            <textarea rows="15" cols="" id="content" style="width:100%; display:none;" name="content"></textarea>
        </form>
        <tr>
            <td colspan="4">
    ...
    <div class="centerbtnwrp">
        <c:choose>
            <c:when test="${searchVO.cmd eq 'write'}">
                <span><a href="javascript:ListBack('${bbsManageVO.bbsId}');" class="abtn grybtn">목록</a></span>
                <span><a href="javascript:writeSubmit(document.writeForm);" class="abtn indbtn">등록</a></span>
            </c:when>
            <c:otherwise>
                <span><a href="javascript:deleteSubmit();" class="abtn whibtn">삭제</a></span>
                <span><a href="javascript:ListBack('${bbsManageVO.bbsId}');" class="abtn grybtn">목록</a></span>
