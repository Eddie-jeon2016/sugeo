<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
    <meta charset="utf-8" />
    <!-- 브라우저별 최상의 렌더링 방식 -->
    <meta http-equiv="X-UA-Compatible" content="IE=Edge">

    <meta name="keywords" content="2018 이통사와 함께하는 개인정보보호캠페인" />
    <meta http-equiv="description" content="국내 영화산업 전문인력 양성을 위해 영화진흥위원회에서 1984년 설립한 영화 전문 교육기관입니다" />
    <META NAME="Keywords" CONTENT="KAFA, 정규과정, 장편과정, 영화인 재교육, 입학, 접수, 마스터클래스, MASTERCLASS">
    <meta name="Publisher" content="(주)포뎁스">
    <meta name="Copyright" content="2018 이통사와 함께하는 개인정보보호캠페인">

    <title>2018 이통사와 함께하는 개인정보보호캠페인</title>

<!--     <script type="text/javascript" language="javascript" src="../js/jquery-1.12.4.min.js"></script> -->
	<script type="text/javascript" language="javascript" src="/js/jquery-3.1.1.min.js"></script>
	<script type="text/javascript" language="javascript" src="/js/jquery-ui/jquery-ui.min.js"></script>
	<script type="text/javascript" language="javascript" src="/js/jquery.form.min.js"></script>
    <script type="text/javascript" language="javascript" src="/js/mng/jquery.bpopup.min.js"></script>
    <script type="text/javascript" language="javascript" src="/js/mng/common.js"></script>
    <link rel="stylesheet" type="text/css" href="/css/mng/front-base.css" media="all">
      
    <!-- <link rel="shortcut icon" type="image/x-icon" href="/images/common/favicon.ico" /> -->  
    <link rel="icon" href="/images/common/favicon/cropped-safety-favicon-32x32.png" sizes="32x32" />
    <link rel="icon" href="/images/common/favicon/cropped-safety-favicon-192x192.png" sizes="192x192" />
    <link rel="apple-touch-icon-precomposed" href="/images/common/favicon/cropped-safety-favicon-180x180.png" />
    <meta name="msapplication-TileImage" content="/images/common/favicon/cropped-safety-favicon-270x270.png" />
    <!-- 170313 파비콘 추가 (s) -->
   <!-- 	<link rel="shortcut icon" type="image/x-icon" href="/images/이미지" /> -->
    <!-- 170313 파비콘 추가 (e) -->
